...
def validate_payment_appid_apptoken(secretsmanager_client, appid, apptoken):
    ...
    logger.info(f"SECRET app_id[{appid}]: {app_id}, app_token[{apptoken}]: {app_token}")
    response = True
    return response
...
def create_3p_order(secretsmanager_client, payment, shipping, headers, create_order_url):
    ...
    app_token = get_or_create_secret(secretsmanager_client, payment["app_token"])
    logger.info(f"SECRET app_id[{payment['app_id']}]: {app_id}, app_token[{payment['app_token']}]: {app_token}")
    json_body = {
    ...
    if(valid_header):
        ...
        response = requests.post(create_order_url, json=json_body, headers=valid_header)
...
def validate_payment(secretsmanager_client, payment, headers, payment_processor_url):
    ...
    if(valid_header):
        logger.info(f"header valid")
        response = requests.post(payment_processor_url, json=json_body, headers=valid_header)
        #response = requests.post(payment_processor_url, json=json_body, headers=headers)
    else:
        logger.info(f"header invalid")
...
def is_payment_valid(secretsmanager_client, payment, headers, payment_processor_url):
    payment_response = validate_payment(secretsmanager_client, payment, headers, payment_processor_url) 
    return payment_response

@tracer.capture_method
def is_payment_valid_appid_apptoken(secretsmanager_client, appid, apptoken):
    payment_response = validate_payment_appid_apptoken(secretsmanager_client, appid, apptoken) 
    return payment_response
