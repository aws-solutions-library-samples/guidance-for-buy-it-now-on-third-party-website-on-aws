...
def cartHandler(event, context):
    ...
    if event["httpMethod"] == "POST":
        ...
        partition_key = f"user_id#{user_id}-cart_id#{cart_id}"
        ...
        #            "#cart_id": "cart_id",
        #        },
        #        UpdateExpression="ADD #quantity :q SET #product=:p, #user_id=:u, #cart_id=:c",
        #        ExpressionAttributeValues={
        #            ':q': quantity, ':p': product, ':u': user_id, ':c': cart_id},
        #        ReturnValues="UPDATED_NEW")
        response = {
            "statusCode": 200,
        ...
        data = buyitnow_table.query(KeyConditionExpression=Key('PK').eq(decoded_cart_id) & Key('SK').begins_with('product#'))
        ...
        data = buyitnow_table.query(KeyConditionExpression=Key('PK').eq(decoded_cart_id) & Key('SK').eq(SK))
